...
def _score_result(score: int, grade: str = "B") -> dict:
    """用于构造模拟的评分结果。"""
    return {
        "success": True,
        "total_score": score,
        "grade": grade,
        "diagnosis": {"verdict": f"总分为 {score} 分"},
    }


def test_build_score_history_returns_empty_for_no_snapshots():
    """用于验证无快照时返回空串。"""
    assert build_score_history(None) == ""
    assert build_score_history([]) == ""


def test_build_score_history_formats_snapshots_as_table():
    """用于验证分数快照被格式化为 Markdown 表格。"""
    snapshots = [
        {"total_score": 68, "grade": "C", "note": "初始评分"},
        {"total_score": 78, "grade": "B", "note": "优化量化"},
    ]
    table = build_score_history(snapshots)
    ...
    assert "68" in table
    assert "78" in table
    assert "初始评分" in table


def test_maybe_record_score_snapshot_skips_non_score_tools():
    """用于验证非评分工具不记录快照。"""
    context: dict = {}
    ResumeToolExecutionStage.maybe_record_score_snapshot(
        tool_name="update_bullet",
        result=_score_result(80),
        executed_tools=[],
        context=context,
    )
    assert "score_snapshots" not in context


def test_maybe_record_score_snapshot_skips_failed_results():
    """用于验证评分失败不记录快照。"""
    context: dict = {}
    ResumeToolExecutionStage.maybe_record_score_snapshot(
...
def test_maybe_record_score_snapshot_records_on_success():
    """用于验证评分成功时记录快照到 context。"""
    context: dict = {}
    ResumeToolExecutionStage.maybe_record_score_snapshot(
        tool_name="score_resume",
        result=_score_result(72, "C"),
        executed_tools=[],
        context=context,
    )
    assert len(context["score_snapshots"]) == 1
    assert context["score_snapshots"][0]["total_score"] == 72
    assert context["score_snapshots"][0]["grade"] == "C"


def test_maybe_record_score_snapshot_accumulates_across_calls():
    """用于验证多次评分会累积快照，形成历史轨迹。"""
    context: dict = {}
    ResumeToolExecutionStage.maybe_record_score_snapshot(
        "score_resume", _score_result(60), [], context,
    )
    ResumeToolExecutionStage.maybe_record_score_snapshot(
        "score_resume", _score_result(75, "B"), [], context,
    )
    ResumeToolExecutionStage.maybe_record_score_snapshot(
        "score_resume", _score_result(83, "B"), [], context,
    )
    snapshots = context["score_snapshots"]
    assert len(snapshots) == 3
    assert [s["total_score"] for s in snapshots] == [60, 75, 83]
