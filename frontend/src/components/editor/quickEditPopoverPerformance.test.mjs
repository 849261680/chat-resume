...
async function readQuickEditSources() {
  ...
  return { popoverSource, editPageSource }
}

test('quick edit prompt input is isolated in QuickEditPopover', async () => {
  const { popoverSource, editPageSource } = await readQuickEditSources()

  assert.match(editPageSource, /<QuickEditPopover/)
