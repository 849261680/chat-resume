...
export function useResumeChatPanel({
   ...
   */
  const dispatchMessage = useCallback(async (messageContent: string) => {
   ...
   */
  const buildMessageContent = useCallback((messageContent: string, context = '') => {
  ...
  const sendMessageWithContext = useCallback(async (context: string, messageContent: string) => {
    await dispatchMessage(buildMessageContent(messageContent, context))
